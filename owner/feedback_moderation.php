...
require_once __DIR__ . '/includes/feedback_page_functions.php'; // moderationStatusMeta(), feedbackPeriodSql(), renderFeedbackPageSwitch()
...
try {
    ...
    $stmt = $pdo->prepare(
        "SELECT f.feedback_id, f.comment, f.moderation_status, f.moderation_reason,
                f.moderation_confidence, f.toxicity_severity, f.sentiment, f.sentiment_confidence, f.sentiment_summary,
...
<title>Feedback Moderation | Owner Panel | OPO! Our Pinoy Original</title>
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                ...
                                >
                                    ...
                                    <?php // Severity is passed too, so a welfare concern reads as "Needs a
                                          // response" instead of sitting under the same "Withheld" word as abuse. ?>
                                    <td><?php $sm = moderationStatusMeta($f['moderation_status'], $f['toxicity_severity'] ?? null); ?><span class="owner-status-pill <?= $sm['class'] ?>"><?= htmlspecialchars($sm['label']) ?></span></td>
...
<script src="assets/js/filter-autosubmit.js?v=<?= filemtime(__DIR__ . '/assets/js/filter-autosubmit.js') ?>"></script>
</body>
</html>
