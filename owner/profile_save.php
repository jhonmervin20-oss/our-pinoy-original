<?php
/**
 * owner/profile_save.php
 *
 * Self-service update for the logged-in owner's OWN first name, last name,
 * and phone only -- always operates on Session::getUserId(), never
 * a posted user_id, so there's no way to edit anyone else's account from
 * here. Email, password, and role are deliberately not accepted by this
 * form at all (see owner/profile.php) -- those stay admin-only changes via
...
try {
    ...
    // immediately instead of showing stale text until the owner logs out.
    ...
    error_log('owner/profile_save.php failed: ' . $e->getMessage());
