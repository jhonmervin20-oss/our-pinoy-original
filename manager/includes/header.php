<?php
/**
 * manager/includes/header.php
 *
 * Top bar for the Manager panel — identical shell to owner/includes/header.php
 * (page title, mobile menu toggle, notifications bell,
 * profile menu incl. the same gold avatar styling — not changed here).
 *
 ...
 * $managerBase = '../'; before requiring this (see sidebar.php).
 */

$pageTitle   = $pageTitle ?? 'Manager panel';
...
// its own per-ingredient Prophet forecasting. That was the old engine, and it
// competed with forecasting/ over the same forecast_runs and
...
//
// sweepPayrollCutoffAlerts() (config/payroll_alerts.php)
// is manager-only -- owner has no payroll workflow to be notified about --
// and is cheap pure date math + a couple of small indexed queries, safe to
// run every load like the inventory sweep. sweepAttendanceThresholdAlerts()
// (config/attendance_alerts.php) is also manager-only and mirrors the same
// shape. All four run before the notifications query just below, so
// anything they create shows up in this very page load's bell, not just
// the next one. Best-effort: a hiccup here should never break the page
// underneath it, matching this file's own existing convention below.
...
try {
    ...
    sweepPayrollCutoffAlerts($sweepPdo);
    sweepAttendanceThresholdAlerts($sweepPdo);
} catch (Throwable $e) {
    error_log('manager/includes/header.php sweep failed: ' . $e->getMessage());
}
...
// Named $headerBellNotifications, not $notifications -- manager/notifications.php
...
$unreadCount = 0;
try {
    $managerNotifPdo = Database::getInstance()->getConnection();
    $managerNotifUserId = Session::getUserId();

    $notifStmt = $managerNotifPdo->prepare(
    ...
    $notifStmt->execute([$managerNotifUserId]);
    $headerBellNotifications = $notifStmt->fetchAll(PDO::FETCH_ASSOC);

    $unreadStmt = $managerNotifPdo->prepare('SELECT COUNT(*) FROM notifications WHERE user_id = ? AND is_read = 0');
    $unreadStmt->execute([$managerNotifUserId]);
    $unreadCount = (int)$unreadStmt->fetchColumn();
} catch (PDOException $e) {
    // no badge, empty dropdown
}

// managerNotifTimeAgo() / managerNotifIcon() / managerNotifLink() now live
// in notification_functions.php, shared with manager/notifications.php
// (the full history page) so both stay on identical icon/link logic.
require_once __DIR__ . '/notification_functions.php';

$managerName    = Session::getFullName() ?: 'Manager';
$managerInitial = Session::getInitials();
$managerRole    = ucfirst(Session::getRole() ?? 'Manager');
?>
<header class="owner-topbar">
    ...
    <div class="owner-topbar-actions">
        ...
        <div class="owner-dropdown" data-owner-dropdown>
            <button type="button" class="owner-icon-btn" data-dropdown-toggle aria-label="Notifications" id="managerNotifToggle">
                <i class="ph ph-bell" aria-hidden="true"></i>
                <span class="owner-badge" id="managerNotifBadge"<?= $unreadCount > 0 ? '' : ' hidden' ?>><?= (int)$unreadCount ?></span>
            </button>

            <div class="owner-dropdown-panel owner-notif-panel">
                <div class="owner-dropdown-header">
                    <span>Notifications</span>
                    <a href="<?= htmlspecialchars($managerBase) ?>notifications_mark_all_read.php?return=<?= urlencode($_SERVER['REQUEST_URI'] ?? '') ?>">Mark all as read</a>
                </div>
                <ul class="owner-notif-list" id="managerNotifList"><?= managerNotifListHtml($headerBellNotifications, $managerBase) ?></ul>
                <a href="<?= htmlspecialchars($managerBase) ?>notifications.php" class="owner-dropdown-footer">View all notifications</a>
            </div>
        </div>

        <div class="owner-dropdown" data-owner-dropdown>
            <button type="button" class="owner-profile-btn" data-dropdown-toggle aria-label="Account menu">
                <span class="owner-avatar"><?= htmlspecialchars($managerInitial) ?></span>
                <span class="owner-profile-btn-text">
                    <span class="owner-profile-btn-name"><?= htmlspecialchars($managerName) ?></span>
                    <span class="owner-profile-btn-role"><?= htmlspecialchars($managerRole) ?></span>
                </span>
            ...
            <div class="owner-dropdown-panel owner-profile-panel">
                <div class="owner-profile-panel-header">
                    <span class="owner-profile-name"><?= htmlspecialchars($managerName) ?></span>
                    <span class="owner-profile-role"><?= htmlspecialchars($managerRole) ?></span>
                </div>
                <div class="owner-dropdown-divider"></div>
                <a href="<?= htmlspecialchars($managerBase) ?>profile.php"><i class="ph ph-user" aria-hidden="true"></i> My profile</a>
                <div class="owner-dropdown-divider"></div>
                <a href="<?= htmlspecialchars($managerBase) ?>../auth/logout.php" class="owner-logout-link"><i class="ph ph-sign-out" aria-hidden="true"></i> Log out</a>
            </div>
        </div>
...
<script src="<?= htmlspecialchars($managerBase) ?>../assets/js/notification-sound.js"></script>
<script>
(function () {
    ...
    const managerBase = <?= json_encode($managerBase) ?>;
    const notifBadge = document.getElementById('managerNotifBadge');
    const notifList = document.getElementById('managerNotifList');
    ...
    function pollNotifications() {
        fetch(managerBase + 'notifications_poll.php?base=' + encodeURIComponent(managerBase), { credentials: 'same-origin' })
